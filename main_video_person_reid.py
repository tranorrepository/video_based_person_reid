...
parser.add_argument('-d', '--dataset', type=str, default='ilidsvid',
                    choices=data_manager.get_names(),help="ilidsvid,mars")
...
# Architecture
parser.add_argument('-a', '--arch', type=str, default='ours', help=" base , base_spatial , base_temporal , ours ")
parser.add_argument('--pool', type=str, default='avg', choices=['avg', 'max'])

# Miscs
parser.add_argument('--print-freq', type=int, default=2, help="print frequency")
...
def main():
    ...
    transform_test = T.Compose([
        T.Scale((args.height, args.width)),
        T.ToTensor(),
        T.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
    ])
    ...
    # elif args.arch=='qan':
    #     model = qan.QAN(args.emb_dim, dataset.num_train_pids, args.cnn)
        # model =selfatt_spp_qan.QAN(args.emb_dim, dataset.num_train_pids, args.cnn)
    # else:
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        if args.eval_step > 0 and (epoch+1) % args.eval_step == 0 or (epoch+1) == args.max_epoch:
            ...
            save_checkpoint({
                ...
                'rank1': rank1,
                'epoch': epoch,
            }, is_best, osp.join(args.save_dir, str(args.arch),'checkpoint_ep' + str(epoch+1) + '.pth.tar'))
...
def test(model, queryloader, galleryloader, pool, use_gpu, ranks=[1, 5, 10]):
    model.eval()
